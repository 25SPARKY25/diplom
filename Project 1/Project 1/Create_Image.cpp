 ...
 Bitmap ^ Create_Image::Cr_image()
{
 ...
 void Create_Image::void_Cr_image(string path)
 {
	 ...
	 switch (fig){
		...
		case 2:	//окружность
			int w = rand() % 250;//размер стороны ограничивающего квадрата
			g->FillRectangle(Whitebrush, 0, 0, 500, 500);//заливаем цветом фон
			g->FillEllipse(Blackbrush, rand() % 250, rand() % 250, w, w);//заливаем цветом окружность
			break;
